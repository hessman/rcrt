...
function setupChartDatas() {
  ...
  for (const item of baseChartData) {
    const currentDomainIndex = getDomainIndex(item.domain);
    const linkWith = ["domain-" + item.queriedDomain];
    for (const d of [...item.linkedDomains]) {
    ...
    perDomainData[currentDomainIndex].children.push({
      id: "dnsName-" + item.dnsName,
      name: item.dnsName,
      value: 20,
      httpStatus: item.httpStatus,
      resolvedIpAddress: item.resolvedIpAddress,
      lastIssuanceDate: item.date ? new Date(item.date) : null,
      linkWith,
    });

    if (item.resolvedIpAddress) {
      let ipIndex = addedIps.get(item.resolvedIpAddress);
      if (ipIndex === undefined) {
        const index =
          perIpData.push({
            id: "ip-" + item.resolvedIpAddress,
            name: item.resolvedIpAddress,
            value: 30,
            linkWith: [],
            children: [],
          }) - 1;
        addedIps.set(item.resolvedIpAddress, index);
        ipIndex = index;
      }
    ...
    if (item.domain !== item.dnsName) {
      const dnsNameWithoutDomain = item.dnsName.replace(
        "." + item.domain,
        ""
      );
